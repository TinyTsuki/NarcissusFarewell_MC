...
public class PlayerAccess {
    ...
    public static PlayerAccess readFromNBT(CompoundTag tag) {
        ...
        tag.getList("blackList").stream()
                .map(Tag::asString)
                .filter(Optional::isPresent)
                .map(Optional::get)
                .forEach(result.blackList::add);

        tag.getList("whiteList").stream()
                .map(Tag::asString)
                .filter(Optional::isPresent)
                .map(Optional::get)
                .forEach(result.whiteList::add);

        tag.getList("autoTpaList").stream()
                .map(Tag::asString)
                .filter(Optional::isPresent)
                .map(Optional::get)
                .forEach(result.autoTpaList::add);

        tag.getList("autoTphList").stream()
                .map(Tag::asString)
                .filter(Optional::isPresent)
                .map(Optional::get)
                .forEach(result.autoTphList::add);
