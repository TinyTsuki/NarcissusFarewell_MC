        ...
        if (page > 0) {
            ...
            for (int i = 0; (page - 1) * ServerConfig.HELP_INFO_NUM_PER_PAGE.get() + i < NarcissusCommand.HELP_MESSAGE.size() && i < ServerConfig.HELP_INFO_NUM_PER_PAGE.get(); i++) {
                ...
                helpInfo.append(Component.literal(com)
                                .clickEvent(new ClickEvent.SuggestCommand(com))
                                .hoverEvent(new HoverEvent.ShowText(Component.trans(NarcissusUtils.getPlayerLanguage(player), EnumI18nType.FORMAT, "click_to_suggest").toTextComponent()))
                        )
            ...
            if (pages > 1) {
                ...
                if (page > 1) {
                    prevButton.color(EnumMCColor.AQUA.getColor())
                            .clickEvent(new ClickEvent.RunCommand(String.format("/%s %s %d", NarcissusUtils.getCommandPrefix(), "help", page - 1)))
                            .hoverEvent(new HoverEvent.ShowText(Component.trans(NarcissusUtils.getPlayerLanguage(player), EnumI18nType.FORMAT, "previous_page").toTextComponent()));
                } else {
                ...
                if (page < pages) {
                    nextButton.color(EnumMCColor.AQUA.getColor())
                            .clickEvent(new ClickEvent.RunCommand(String.format("/%s %s %d", NarcissusUtils.getCommandPrefix(), "help", page + 1)))
                            .hoverEvent(new HoverEvent.ShowText(Component.trans(NarcissusUtils.getPlayerLanguage(player), EnumI18nType.FORMAT, "next_page").toTextComponent()));
                } else {
