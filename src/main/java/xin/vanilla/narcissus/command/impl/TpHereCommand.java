...
public final class TpHereCommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (!ignore) {
            Component yesButton = Component.trans(NarcissusUtils.getPlayerLanguage(target), EnumI18nType.FORMAT, "yes_button", NarcissusUtils.getPlayerLanguage(target))
                    .clickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format("/%s %s %s", NarcissusUtils.getCommandPrefix(), CommonConfig.COMMAND_TP_HERE_YES.get(), request.getRequestId())));
            Component noButton = Component.trans(NarcissusUtils.getPlayerLanguage(target), EnumI18nType.FORMAT, "no_button", NarcissusUtils.getPlayerLanguage(target))
                    .clickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format("/%s %s %s", NarcissusUtils.getCommandPrefix(), CommonConfig.COMMAND_TP_HERE_NO.get(), request.getRequestId())));
            NarcissusUtils.sendMessage(target, Component.trans(NarcissusUtils.getPlayerLanguage(target), EnumI18nType.FORMAT, "tp_here_request_received",
                    player.getDisplayName().getString(), Component.trans(NarcissusUtils.getPlayerLanguage(target), EnumI18nType.WORD, request.isSafe() ? "tp_here_safe" : "tp_here_unsafe"), yesButton, noButton));
        }
        Component cancelButton = Component.trans(NarcissusUtils.getPlayerLanguage(target), EnumI18nType.FORMAT, "cancel_button", NarcissusUtils.getPlayerLanguage(target))
                .clickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format("/%s %s %s", NarcissusUtils.getCommandPrefix(), CommonConfig.COMMAND_TP_HERE_CANCEL.get(), request.getRequestId())));
        NarcissusUtils.sendMessage(player, Component.trans(NarcissusUtils.getPlayerLanguage(player), EnumI18nType.FORMAT, "tp_here_request_sent", target.getDisplayName().getString(), cancelButton));
        if (autoAccept) {
    ...
    public static LiteralArgumentBuilder<CommandSourceStack> create() {
        return Commands.literal(CommonConfig.COMMAND_TP_HERE.get())
