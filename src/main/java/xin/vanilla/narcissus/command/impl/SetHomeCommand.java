    ...
    private static int execute(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (defaultHome) {
            if (data.getDefaultHome().containsKey(player.level().dimension().identifier().toString())) {
                NarcissusUtils.sendTranslatableMessage(player, I18nUtils.getKey(EnumI18nType.FORMAT, "home_default_remove"), data.getDefaultHome(player.level().dimension().identifier().toString()).value());
            }
