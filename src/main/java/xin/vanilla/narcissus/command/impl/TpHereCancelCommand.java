...
public final class TpHereCancelCommand {
    ...
    private static int execute(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        NarcissusNotificationSend.send(request.getRequester(), NarcissusComponent.get().transAuto("tp_here_cancelled", request.getRequester().getDisplayName().getString()), NarcissusNotificationTypes.TELEPORT_REQUEST);
        if (!request.isIgnore() && !request.getRequester().getUUID().equals(request.getTarget().getUUID())) {
            NarcissusNotificationSend.send(request.getTarget(), NarcissusComponent.get().transAuto("tp_here_cancelled", request.getRequester().getDisplayName().getString()), NarcissusNotificationTypes.TELEPORT_REQUEST);
        }
        return 1;
    }
