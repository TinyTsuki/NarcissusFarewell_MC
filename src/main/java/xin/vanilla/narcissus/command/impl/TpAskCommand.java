    ...
    private static int execute(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (!ignore) {
            Component yesButton = Component.trans(NarcissusUtils.getPlayerLanguage(target), EnumI18nType.FORMAT, "yes_button", NarcissusUtils.getPlayerLanguage(target))
                    .clickEvent(new ClickEvent.RunCommand(String.format("/%s %s %s", NarcissusUtils.getCommandPrefix(), CommonConfig.COMMAND_TP_ASK_YES.get(), request.getRequestId())));
            Component noButton = Component.trans(NarcissusUtils.getPlayerLanguage(target), EnumI18nType.FORMAT, "no_button", NarcissusUtils.getPlayerLanguage(target))
                    .clickEvent(new ClickEvent.RunCommand(String.format("/%s %s %s", NarcissusUtils.getCommandPrefix(), CommonConfig.COMMAND_TP_ASK_NO.get(), request.getRequestId())));
        ...
        Component cancelButton = Component.trans(NarcissusUtils.getPlayerLanguage(target), EnumI18nType.FORMAT, "cancel_button", NarcissusUtils.getPlayerLanguage(target))
                .clickEvent(new ClickEvent.RunCommand(String.format("/%s %s %s", NarcissusUtils.getCommandPrefix(), CommonConfig.COMMAND_TP_ASK_CANCEL.get(), request.getRequestId())));
