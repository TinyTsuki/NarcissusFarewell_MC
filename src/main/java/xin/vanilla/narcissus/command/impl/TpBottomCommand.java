...
public final class TpBottomCommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        Coordinate coordinate = NarcissusUtils.findBottomCandidate(player, new Coordinate(player));
        if (coordinate == null) {
            NarcissusUtils.sendTranslatableMessage(player, I18nUtils.getKey(EnumI18nType.FORMAT, "tp_bottom_not_found"));
            return 0;
        }
        coordinate.safe("safe".equalsIgnoreCase(CommandUtils.getStringDefault(context, "safe", "safe"))).safeMode(EnumSafeMode.Y_B_TO_C);
        if (CommandUtils.checkTeleportPost(player, coordinate, EnumTeleportType.TP_BOTTOM, true)) return 0;
        NarcissusUtils.teleportTo(player, coordinate, EnumTeleportType.TP_BOTTOM);
        return 1;
    }

    public static LiteralArgumentBuilder<CommandSourceStack> create() {
        return Commands.literal(CommonConfig.COMMAND_TP_BOTTOM.get())
