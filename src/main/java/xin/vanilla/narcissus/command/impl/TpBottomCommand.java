...
public final class TpBottomCommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (safeWorldCoordinate == null) {
            MessageUtils.sendMessage(player, NarcissusComponent.get().transAuto("tp_bottom_not_found"));
            return 0;
        }
        safeWorldCoordinate.safe("safe".equalsIgnoreCase(CommandUtils.getStringDefault(context, "safe", "safe"))).safeMode(EnumSafeMode.Y_B_TO_C);
