...
public class FarewellCommand {
     ...
     * @param isTarget     是否根据接收方查找
     */
    private static SuggestionProvider<CommandSource> buildReqIndexSuggestions(ETeleportType teleportType, final boolean isTarget) {
     ...
     * @return true 表示校验失败，不应该执行传送
     */
    private static boolean checkTeleportPost(ServerPlayerEntity player, Coordinate target, ETeleportType type) {
     ...
     * @return true 表示校验失败，不应该执行传送
     */
    private static boolean checkTeleportPost(ServerPlayerEntity player, Coordinate target, ETeleportType type, boolean submit) {
        boolean result;
        // 判断跨维度传送
