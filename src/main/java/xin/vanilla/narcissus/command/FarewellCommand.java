    ...
    */
    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        ...
        Command<CommandSource> dimCommand = (context) -> {
            ServerPlayerEntity player = context.getSource().getPlayerOrException();
            Component msg = Component.translatable(NarcissusUtils.getPlayerLanguage(player), EI18nType.MESSAGE, "dimension_info", player.getLevel().dimension.getType().getRegistryName().toString());
        ...
        Command<CommandSource> feedCommand = (context) -> {
            ...
            for (ServerPlayerEntity target : targetList) {
                if (NarcissusUtils.killPlayer(target)) {
                    NarcissusUtils.broadcastMessage(player, Component.translatable(NarcissusUtils.getPlayerLanguage(player), EI18nType.MESSAGE, "died_of_narcissus_" + (new Random().nextInt(4) + 1), target.getDisplayName().getString()));
                }
            }
            return 1;
        };
        ...
        Command<CommandSource> tpStructureCommand = context -> {
            ...
            player.connection.send(new SChatPacket(Component.translatable(NarcissusUtils.getPlayerLanguage(player), EI18nType.MESSAGE, "tp_structure_searching").toTextComponent(), ChatType.GAME_INFO));
            new Thread(() -> {
                Coordinate coordinate;
                if (biome != null) {
        ...
        Command<CommandSource> tpViewCommand = context -> {
            ...
            player.connection.send(new SChatPacket(Component.translatable(NarcissusUtils.getPlayerLanguage(player), EI18nType.MESSAGE, "tp_view_searching").toTextComponent(), ChatType.GAME_INFO));
        ...
        Command<CommandSource> tpHomeCommand = context -> {
            ...
            if (coordinate == null) {
                if (targetLevel == null && name == null) {
                    ...
                    NarcissusUtils.sendTranslatableMessage(player, I18nUtils.getKey(EI18nType.MESSAGE, "home_not_found_in_dimension"), targetLevel.getRegistryName().toString());
                    ...
                    NarcissusUtils.sendTranslatableMessage(player, I18nUtils.getKey(EI18nType.MESSAGE, "home_not_found_with_name_in_dimension"), targetLevel.getRegistryName().toString(), name);
                }
                return 0;
            }
            try {
        ...
        Command<CommandSource> setHomeCommand = context -> {
            ...
            if (defaultHome) {
                if (data.getDefaultHome().containsKey(player.getLevel().dimension.getType().getRegistryName().toString())) {
                    NarcissusUtils.sendTranslatableMessage(player, I18nUtils.getKey(EI18nType.MESSAGE, "home_default_remove"), data.getDefaultHome(player.getLevel().dimension.getType().getRegistryName().toString()).getValue());
                }
