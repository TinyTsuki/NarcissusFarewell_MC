...
public final class AbstractGuiUtils {
    ...
    public static void blit(PoseStack stack, int x0, int y0, int z, int destWidth, int destHeight, TextureAtlasSprite sprite) {
        GuiComponent.blit(stack, x0, y0, z, destWidth, destHeight, sprite);
    }

    public static void blitBlend(PoseStack stack, int x0, int y0, int z, int destWidth, int destHeight, TextureAtlasSprite sprite) {
        blitByBlend(() ->
                GuiComponent.blit(stack, x0, y0, z, destWidth, destHeight, sprite)
        );
    }

    public static void blit(PoseStack stack, int x0, int y0, int z, double u0, double v0, int width, int height, int textureHeight, int textureWidth) {
        GuiComponent.blit(stack, x0, y0, z, (float) u0, (float) v0, width, height, textureHeight, textureWidth);
    }

    public static void blitBlend(PoseStack stack, int x0, int y0, int z, double u0, double v0, int width, int height, int textureHeight, int textureWidth) {
        blitByBlend(() ->
                GuiComponent.blit(stack, x0, y0, z, (float) u0, (float) v0, width, height, textureHeight, textureWidth)
        );
    }

    public static void blit(PoseStack stack, int x0, int y0, int destWidth, int destHeight, double u0, double v0, int srcWidth, int srcHeight, int textureWidth, int textureHeight) {
        GuiComponent.blit(stack, x0, y0, destWidth, destHeight, (float) u0, (float) v0, srcWidth, srcHeight, textureWidth, textureHeight);
    }

    public static void blitBlend(PoseStack stack, int x0, int y0, int destWidth, int destHeight, double u0, double v0, int srcWidth, int srcHeight, int textureWidth, int textureHeight) {
        blitByBlend(() ->
                GuiComponent.blit(stack, x0, y0, destWidth, destHeight, (float) u0, (float) v0, srcWidth, srcHeight, textureWidth, textureHeight)
        );
    }

    public static void blit(PoseStack stack, int x0, int y0, double u0, double v0, int destWidth, int destHeight, int textureWidth, int textureHeight) {
    ...
    public static void drawLimitedText(@Nonnull FontDrawArgs args) {
        ...
        if (StringUtils.isNotNullOrEmpty(text.content())) {
            ...
            for (int index = 0; index < processedLines.length; index++) {
                ...
                if (hasShadow) {
                    font.drawShadow(stack, lineText.toComponent().toTextComponent(I18nUtils.getClientLanguage()), (float) drawX + xOffset, yPos, textColor);
                } else {
                    font.draw(stack, lineText.toComponent().toTextComponent(I18nUtils.getClientLanguage()), (float) drawX + xOffset, yPos, textColor);
                }
            }
    ...
    public static void renderItem(PoseStack stack, ItemRenderer itemRenderer, ItemStack itemStack, int x, int y, boolean showText) {
        AbstractGuiUtils.renderItem(stack, itemRenderer, getFont(), itemStack, x, y, showText);
    }

    public static void renderItem(PoseStack stack, ItemRenderer itemRenderer, Font font, ItemStack itemStack, int x, int y, boolean showText) {
        itemRenderer.renderGuiItem(stack, itemStack, x, y);
        if (showText) {
            itemRenderer.renderGuiItemDecorations(stack, font, itemStack, x, y, String.valueOf(itemStack.getCount()));
        }
    }
    ...
    private static void drawPolygonShape(ShapeDrawArgs args) {
        ...
        if (polygon.border() > 0) {
            drawPolygonBorder(stack, polygon, color);
        } else {
            drawPolygon(stack, polygon.centerX(), polygon.centerY(), polygon.radius(),
     ...
     * 绘制实心扇环
     */
    private static void drawFilledSectorRing(PoseStack stack, ShapeDrawArgs.SectorRingParams params, float innerRadius, int segments, int color) {
     ...
     * 绘制实心扇环
     */
    private static void drawFilledSectorRingFromCenter(PoseStack stack, ShapeDrawArgs.SectorRingParams params, int segments, int color) {
        float centerX = params.centerX();
        float centerY = params.centerY();
     ...
     * @param color       边框颜色
     */
    private static void drawRoundedRectOutLine(PoseStack stack, float x, float y, float width, float height,
     ...
     * 绘制多边形边框
     */
    private static void drawPolygonBorder(PoseStack stack, ShapeDrawArgs.PolygonParams polygon, int color) {
    ...
    public static void drawNinePatch(PoseStack stack, Texture texture, int x, int y, int destWidth, int destHeight, double scale) {
        ...
        if (info == null) {
            blit(stack, x, y, destWidth, destHeight,
    ...
    public static void drawPopupMessage(FontDrawArgs args) {
        ...
        AbstractGuiUtils.renderByDepth(args.text().stack(), EnumRenderDepth.POPUP_TIPS, (stack) -> {
            ...
            if (bgArgs.texture() != null && ninePatchInfo != null) {
                ...
                bindTexture(tex.location());
                blit(stack, (int) bgArgs.x(), (int) bgArgs.y(), finalMsgWidth, finalMsgHeight,
    ...
    public static void drawItemTooltip(PoseStack stack, ItemStack itemStack, double x, double y) {
        ...
        FontDrawArgs drawArgs = FontDrawArgs.ofPopo(tooltipText.stack(stack))
                .x(x).y(y);
