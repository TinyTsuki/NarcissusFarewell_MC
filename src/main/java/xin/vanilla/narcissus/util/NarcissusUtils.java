...
import xin.vanilla.narcissus.config.TeleporterCustom;
...
public class NarcissusUtils {
  ...
  public static Coordinate findViewEndCandidate(EntityPlayerMP player, boolean safe, int range) {
    ...
    // 步长
    Vec3 stepVector = Vec3.createVectorHelper(direction.xCoord * stepScale,
        direction.yCoord * stepScale, direction.zCoord * stepScale);
    ...
    for (int stepCount = 0; stepCount <= range; stepCount++) {
      // 更新当前检测位置
      currentPosition = startPosition.addVector(stepVector.xCoord * stepCount,
          stepVector.yCoord * stepCount, stepVector.zCoord * stepCount);

      // 获取当前方块状态
      Block blockState = world.getBlock(ceil(currentPosition.xCoord),
          (int) currentPosition.yCoord, ceil(currentPosition.zCoord));

      // 检测方块是否不可穿过
      if (blockState.getMaterial().blocksMovement()) {
        result =
            start.clone().fromVector3(startPosition.addVector(stepVector.xCoord * (stepCount - 1)
                , stepVector.yCoord * (stepCount - 1), stepVector.zCoord * (stepCount - 1)));
        break;
      }
    }
    ...
    if (safe) {
      ...
      for (int stepCount =
           (int) Math.ceil(collisionVector.distanceTo(startPosition) / stepScale); stepCount >= 0; stepCount--) {
        currentPosition = startPosition.addVector(stepVector.xCoord * stepCount,
            stepVector.yCoord * stepCount, stepVector.zCoord * stepCount);
  ...
  public static Coordinate findSafeCoordinate(Coordinate coordinate, boolean belowAllowAir) {
    World world =
        DimensionManager.getWorld(DimensionUtils.getDimensionType(coordinate.getDimension()));
  ...
  private static Coordinate searchForSafeCoordinateInChunk(World world, Coordinate coordinate,
                                                           int chunkX, int chunkZ,
                                                           boolean belowAllowAir) {
    // 搜索安全位置，限制在目标范围区块内
    ...
    if (coordinate.getSafeMode() == ESafeMode.Y_DOWN) {
      ...
      IntStream.range(chunkMinX, chunkMaxX)
          .forEach(x -> IntStream.range(chunkMinZ, chunkMaxZ)
              .forEach(z -> IntStream.range(NarcissusUtils.getWorldMinY(world),
                      NarcissusUtils.getWorldMaxY(world))
  ...
  private static boolean isAirCoordinate(World world, Coordinate coordinate) {
    Block block = world.getBlock(ceil(coordinate.getX()), (int) coordinate.getY(),
        ceil(coordinate.getZ()));
    Block blockAbove = world.getBlock(ceil(coordinate.getX()), (int) coordinate.getY() + 1,
        ceil(coordinate.getZ()));
    Block blockBelow = world.getBlock(ceil(coordinate.getX()), (int) coordinate.getY() - 1,
        ceil(coordinate.getZ()));
   ...
   */
  private static boolean isSafeBlock(World world, Coordinate coordinate, boolean belowAllowAir,
   ...
   */
  public static Coordinate findNearestBiome(WorldServer world, Coordinate start,
                                            BiomeGenBase biome, int radius, int minDistance) {
   ...
   * @param radius 搜索半径
   */
  public static Coordinate findNearestStruct(WorldServer world, Coordinate start, String struct,
                                             int radius) {
    ChunkPosition pos = world.findClosestStructure(struct, (int) start.getX(), (int) start.getY()
        , (int) start.getZ());
    if (pos != null) {
  ...
  public static List<String> getStructureList() {
    if (structureList.isEmpty()) {
      try {
        Field field =
            MapGenStructureIO.class.getDeclaredField(FieldUtils.getStartNameToClassMapFieldName());
  ...
  public static KeyValue<String, String> getPlayerHomeKey(EntityPlayerMP player, String dimension
      , String name) {
    IPlayerTeleportData data = PlayerTeleportData.get(player);
    ...
    if (StringUtils.isNullOrEmptyEx(dimension) && StringUtils.isNotNullOrEmpty(name)) {
      if (defaultHome.isEmpty() || !defaultHome.containsValue(name)) {
        ...
        List<Map.Entry<String, String>> entryList =
            defaultHome.entrySet().stream().filter(entry -> entry.getValue().equals(name)).collect(Collectors.toList());
        if (entryList.size() == 1) {
      ...
      if (defaultHome.containsKey(DimensionUtils.getStringId(dimension))) {
        keyValue = new KeyValue<>(DimensionUtils.getStringId(dimension),
            defaultHome.get(DimensionUtils.getStringId(dimension)));
      }
      ...
      if (defaultHome.size() == 1) {
        keyValue = new KeyValue<>(defaultHome.keySet().iterator().next(),
            defaultHome.values().iterator().next());
      } else {
        String value =
            defaultHome.getOrDefault(DimensionUtils.getStringId(player.getEntityWorld().provider)
                , null);
        if (value != null) {
          keyValue = new KeyValue<>(DimensionUtils.getStringId(player.getEntityWorld().provider),
              value);
        }
      }
   ...
   */
  public static TeleportRecord getBackTeleportRecord(EntityPlayerMP player,
                                                     @Nullable ETeleportType type,
  ...
  public static void teleportTo(@NonNull TeleportRequest request) {
    teleportTo(request.getRequester(), request.getTarget(), request.getTeleportType(),
        request.isSafe());
  }
   ...
   * @param to   目标玩家
   */
  public static void teleportTo(@NonNull EntityPlayerMP from, @NonNull EntityPlayerMP to,
                                ETeleportType type, boolean safe) {
   ...
   * @param after  坐标
   */
  public static void teleportTo(@NonNull EntityPlayerMP player, @NonNull Coordinate after,
                                ETeleportType type) {
    final Coordinate before = new Coordinate(player);
    final World world = player.getEntityWorld();
    final WorldServer level =
        DimensionManager.getWorld(DimensionUtils.getDimensionType(after.getDimension()));

    if (Objects.isNull(world) || Objects.isNull(level)) {
      return;
    }
    ...
    NarcissusUtils.sendMessage(player, Component.translatable(
        NarcissusUtils.getPlayerLanguage(player), EI18nType.MESSAGE, "safe_searching"));

    new Thread(() -> {
      final Coordinate finalAfter = findSafeCoordinate(after.clone(), false);
      final Coordinate airCoordinate = findSafeCoordinate(finalAfter, true);
      final List<ItemStack> playerItemList = getPlayerItemList(player);
  ...
  private static void teleportPlayer(@NonNull EntityPlayerMP player, @NonNull Coordinate after,
                                     ETeleportType type, Coordinate before, WorldServer level) {
   ...
   * @return 玩家的坐骑
   */
  private static @Nullable Entity teleportPassengers(EntityPlayerMP player, Entity parent,
                                                     Entity passenger,
   ...
   * 传送跟随的实体
   */
  private static void teleportFollowers(@NonNull EntityPlayerMP player,
                                        @NonNull Coordinate coordinate, WorldServer level) {
    if (!ServerConfig.TP_WITH_FOLLOWER) return;

    int followerRange = ServerConfig.TP_WITH_FOLLOWER_RANGE;

    // 传送主动跟随的实体
    for (Object obj : player.getServerForPlayer().getEntitiesWithinAABB(EntityTameable.class,
        player.boundingBox.expand(followerRange, followerRange, followerRange))) {
    ...
    // 传送拴绳实体
    for (Object obj : player.worldObj.getEntitiesWithinAABB(EntityLiving.class,
        player.boundingBox.expand(followerRange, followerRange, followerRange))) {
      ...
      if (entity.canEntityBeSeen(player) &&
          taskEntries.stream()
              .anyMatch(taskObj -> (taskObj.action instanceof EntityAITempt)
                  ...
                  && FieldUtils.getPrivateFieldValue(EntityAITempt.class, taskObj.action,
                  FieldUtils.getTemptGoalPlayerFieldName()) == player
              )) {
        doTeleport(entity, coordinate, level);
      }
    }
  ...
  private static void doTeleport(@NonNull Entity entity, @NonNull Coordinate coordinate,
                                 WorldServer level) {
    if (!(entity instanceof EntityPlayerMP)) {
      if (!DimensionUtils.getStringId(entity.dimension).equalsIgnoreCase(coordinate.getDimension())) {
        NarcissusFarewell.getServerInstance().getConfigurationManager()
            .transferEntityToWorld(entity,
                DimensionUtils.getDimensionType(coordinate.getDimension())
                , DimensionUtils.getWorldByDimensionId(entity.dimension), level
                , new TeleporterCustom(level));
      }
    ...
    player.cameraPitch = coordinate.getPitch() == 0 ? player.cameraPitch :
        (float) coordinate.getPitch();
  }
  ...
  public static EntityPlayerMP getRandomPlayer() {
    try {
      List<EntityPlayerMP> players =
          NarcissusFarewell.getServerInstance().getConfigurationManager().playerEntityList;
   ...
   * @return 是否全部移除成功
   */
  public static boolean removeItemFromPlayerInventory(EntityPlayerMP player,
  ...
  public static List<ItemStack> getPlayerItemList(EntityPlayerMP player) {
    ...
    if (player != null) {
      ...
      result =
          result.stream().filter(itemStack -> itemStack != null && itemStack.stackSize > 0).collect(Collectors.toList());
    }
    return result;
  }
  ...
  public static void sendMessage(EntityPlayerMP player, Component message) {
    for (IChatComponent component :
        message.toChatComponent(NarcissusUtils.getPlayerLanguage(player))) {
  ...
  public static void sendTranslatableMessage(EntityPlayerMP player, String key, Object... args) {
    for (IChatComponent component :
        Component.translatable(key, args).setLanguageCode(NarcissusUtils.getPlayerLanguage(player)).toChatComponent()) {
      player.addChatMessage(component);
    }
  }
   ...
   * @param args    参数
   */
  public static void sendTranslatableMessage(ICommandSender source, boolean success, String key,
                                             Object... args) {
    if (source instanceof EntityPlayerMP) {
      sendTranslatableMessage((EntityPlayerMP) source, key, args);
    } else {
      for (IChatComponent component :
          Component.translatable(key, args).setLanguageCode(ServerConfig.DEFAULT_LANGUAGE).toChatComponent()) {
  ...
  public static boolean isTeleportAcrossDimensionEnabled(EntityPlayerMP player, String to,
                                                         ETeleportType type) {
    boolean result = true;
    if (!DimensionUtils.getStringId(player.getEntityWorld().provider.dimensionId).equalsIgnoreCase(to)) {
      if (ServerConfig.TELEPORT_ACROSS_DIMENSION) {
        if (!NarcissusUtils.isTeleportTypeAcrossDimensionEnabled(player, type)) {
          result = false;
          NarcissusUtils.sendTranslatableMessage(player, I18nUtils.getKey(EI18nType.MESSAGE,
              "across_dimension_not_enable_for"), getCommand(type));
        }
      } else {
        result = false;
        NarcissusUtils.sendTranslatableMessage(player, I18nUtils.getKey(EI18nType.MESSAGE,
            "across_dimension_not_enable"));
      }
    }
    return result;
  }
   ...
   * 判断传送类型跨维度传送是否开启
   */
  public static boolean isTeleportTypeAcrossDimensionEnabled(EntityPlayerMP player,
                                                             ETeleportType type) {
    int permission;
    switch (type) {
      case TP_COORDINATE:
  ...
  public static int getTeleportCoolDown(EntityPlayerMP player, ETeleportType type) {
    ...
    switch (ServerConfig.TELEPORT_REQUEST_COOLDOWN_TYPE) {
      case COMMON:
        return calculateCooldown(player.getUniqueID(), current, lastTpTime,
            ServerConfig.TELEPORT_REQUEST_COOLDOWN, null);
      case INDIVIDUAL:
      ...
      case MIXED:
        ...
        int individualCooldown = calculateCooldown(player.getUniqueID(), current, lastTpTime,
            commandCoolDown, type);
        int globalCooldown = calculateCooldown(player.getUniqueID(), current, lastTpTime,
            globalCommandCoolDown, null);
  ...
  private static int calculateCooldown(UUID uuid, Instant current, Instant lastTpTime,
    ...
    return Math.max(0,
        Math.max(cooldown - (int) Duration.between(lastRequestTime, current).getSeconds(),
            cooldown - (int) Duration.between(lastTpTime, current).getSeconds()));
  }
   ...
   * @return 是否验证通过
   */
  public static boolean validTeleportCost(EntityPlayerMP player, Coordinate target,
                                          ETeleportType type, boolean submit) {
    return validateCost(player, target.getDimension(), calculateDistance(new Coordinate(player),
        target), type, submit);
  }
  ...
  public static boolean validTeleportCost(TeleportRequest request, boolean submit) {
    ...
    return validateCost(request.getRequester(),
        DimensionUtils.getStringId(request.getTarget().getEntityWorld().provider.dimensionId),
        calculateDistance(requesterCoordinate, targetCoordinate), request.getTeleportType(),
        submit);
  }
   ...
   * @return 是否验证通过
   */
  private static boolean validateCost(EntityPlayerMP player, String targetDim, double distance,
                                      ETeleportType teleportType, boolean submit) {
      ...
      case ITEM:
        try {
          ...
          if (StringUtils.isNotNullOrEmpty(nbt)) {
            try {
              itemStack.setTagCompound((NBTTagCompound) JsonToNBT.func_150315_a(nbt));
            } catch (Exception ignored) {
            }
          }
      ...
      case COMMAND:
        try {
          ...
          if (result && submit) {
            ...
            int commandResult =
                NarcissusFarewell.getServerInstance().getCommandManager().executeCommand(player,
                    command);
            if (commandResult > 0) {
              data.subTeleportCard(Math.min(data.getTeleportCard(), cardNeed));
            }
  ...
  public static int getTeleportCostNeed(IPlayerTeleportData data, int card, int need) {
    if (!ServerConfig.TELEPORT_CARD) return need;
    switch (ServerConfig.TELEPORT_CARD_TYPE) {
      case NONE:
        // card = 1
  ...
  public static String getServerPlayerLanguage(EntityPlayerMP player) {
    Object value = FieldUtils.getPrivateFieldValue(EntityPlayerMP.class, player,
        FieldUtils.getPlayerLanguageFieldName(player));
    if (value == null) {
      return "en_us";
    }
    return (String) value;
   ...
   */
  public static void clonePlayerLanguage(EntityPlayerMP originalPlayer,
                                         EntityPlayerMP targetPlayer) {
    FieldUtils.setPrivateFieldValue(EntityPlayerMP.class, targetPlayer,
        FieldUtils.getPlayerLanguageFieldName(originalPlayer),
        getServerPlayerLanguage(originalPlayer));
  }
  ...
  public static boolean killPlayer(EntityPlayerMP player) {
    try {
      ...
      if (!player.getEntityWorld().getGameRules().getGameRuleBooleanValue("keepInventory")) {
        ...
        PlayerDropsEvent event = new PlayerDropsEvent(player, DAMAGE_SOURCE, player.capturedDrops
            , false);
