...
public final class ItemUtils {
    ...
    public static ItemStack deserializeItemStack(String itemString) {
        ...
        ItemStack cached = deserializeCache.computeIfAbsent(itemString, k -> {
            try {
                ItemParser.ItemResult itemResult = ItemParser.parseForItem(BuiltInRegistries.ITEM.asLookup(), new StringReader(k));
                ItemStack result = new ItemInput(itemResult.item(), itemResult.nbt()).createItemStack(1, false);
                return result.copy();
            } catch (Exception e) {
    ...
    private static List<ItemStack> buildAllItemsList() {
        ...
        try {
            ...
            try {
                CreativeModeTab searchTab = CreativeModeTabs.SEARCH;
            ...
            try {
                for (CreativeModeTab group : CreativeModeTabs.allTabs()) {
                    if (group == null || group == CreativeModeTabs.SEARCH) continue;
                    try {
