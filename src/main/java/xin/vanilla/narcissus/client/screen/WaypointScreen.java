...
public class WaypointScreen extends Screen {
    ...
    public void render(@Nonnull GuiGraphics graphics, int mouseX, int mouseY, float tick) {
        ...
        // 绘制三个面板
        drawPanel(graphics, startX, startY, Component.transClient(EnumI18nType.WORD, "private").toString(), homeItems, homeScroll, mouseX, mouseY, false);
        drawPanel(graphics, startX + panelWidth + GAP_H, startY, Component.transClient(EnumI18nType.WORD, "public").toString(), stageItems, stageScroll, mouseX, mouseY, false);
        drawPanel(graphics, startX + (panelWidth + GAP_H) * 2, startY, Component.transClient(EnumI18nType.WORD, "footprints").toString(), backItems, backScroll, mouseX, mouseY, true);

        // 底部详情区
        int footerY = startY + panelHeight + GAP_V_FOOTER;
        drawFooter(graphics, footerY, mouseX, mouseY);

        // 传送卡数量
        String ticketStr = Component.transClient(EnumI18nType.WORD, "teleport_card").toString() + ": " + ticketCount;
        int tw = font.width(ticketStr);
        graphics.drawString(font, ticketStr, width - tw - 10, 10, COLORS_TEXT_ON_DARK, false);

        // 删除确认遮罩
        if (deleteConfirmItem != null) {
            drawDeleteConfirmOverlay(graphics, mouseX, mouseY);
        }

        // Tooltip
        if (hoveredItem != null && deleteConfirmItem == null) {
            drawCustomTooltip(graphics, hoveredItem, mouseX, mouseY);
        }

        // 传送按钮
        drawTeleportButton(graphics, mouseX, mouseY);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double deltaX, double deltaY) {
        ...
        for (int p = 0; p < 3; p++) {
            ...
            if (mouseX >= px && mouseX < px + contentW && mouseY >= listY && mouseY < listY + listHeight) {
                if (p == 0) {
                    homeScroll = (int) clamp(homeScroll - deltaY, 0, Math.max(0, homeItems.size() - MAX_VISIBLE_ITEMS));
                } else if (p == 1) {
                    stageScroll = (int) clamp(stageScroll - deltaY, 0, Math.max(0, stageItems.size() - MAX_VISIBLE_ITEMS));
                } else {
                    backScroll = (int) clamp(backScroll - deltaY, 0, Math.max(0, backItems.size() - MAX_VISIBLE_ITEMS));
                }
                return true;
            }
        }
    ...
    private void drawTeleportButton(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        AbstractGuiUtils.drawRoundedRect(graphics, buttonX, buttonY, buttonW, buttonH, bgColor, 4);
        AbstractGuiUtils.drawRoundedRectOutLineRough(graphics, buttonX, buttonY, buttonW, buttonH, 1, borderColor, 4);
        String btnText = Component.transClient(EnumI18nType.WORD, "teleport_btn").toString();
        graphics.drawString(font, btnText, buttonX + (buttonW - font.width(btnText)) / 2, buttonY + (buttonH - font.lineHeight) / 2 + 1, textColor, false);
    }

    private void drawCustomTooltip(GuiGraphics graphics, WaypointEntry item, int mouseX, int mouseY) {
        ...
        Text tooltipText = Text.literal(content).graphics(graphics).font(font).color(Color.argb(COLORS_TEXT));
    ...
    private void drawPanel(GuiGraphics graphics, int x, int y, String title, List<WaypointEntry> items, int scroll, int mouseX, int mouseY, boolean isBackPanel) {
        AbstractGuiUtils.drawRoundedRect(graphics, x, y, panelWidth, panelHeight, COLORS_BG_PANEL, 3);
        AbstractGuiUtils.drawRoundedRectOutLineRough(graphics, x, y, panelWidth, panelHeight, 1, COLORS_BORDER, 3);
        AbstractGuiUtils.fill(graphics, x + 2, y + TITLE_HEIGHT - 1, panelWidth - 4, 1, COLORS_BORDER_SOFT);

        graphics.drawString(font, title, x + PANEL_PADDING, y + 3, COLORS_TEXT, false);
        ...
        for (int i = 0; i < MAX_VISIBLE_ITEMS; i++) {
            ...
            AbstractGuiUtils.drawRoundedRect(graphics, listX, itemY, contentW, ITEM_HEIGHT - 1, bgColor, 2);
            if (item.canTeleport) {
                if (selected) {
                    AbstractGuiUtils.fill(graphics, listX, itemY, 3, ITEM_HEIGHT - 1, COLORS_ACCENT);
                    AbstractGuiUtils.drawRoundedRectOutLineRough(graphics, listX, itemY, contentW, ITEM_HEIGHT - 1, 1, COLORS_ACCENT_SOFT, 2);
                } else if (hover) {
                    AbstractGuiUtils.fill(graphics, listX, itemY, 2, ITEM_HEIGHT - 1, COLORS_ACCENT_SOFT);
                }
            } else if (selected) {
                AbstractGuiUtils.fill(graphics, listX, itemY, 3, ITEM_HEIGHT - 1, COLORS_SELECTED_DISABLED);
                AbstractGuiUtils.drawRoundedRectOutLineRough(graphics, listX, itemY, contentW, ITEM_HEIGHT - 1, 1, COLORS_SELECTED_DISABLED, 2);
            }

            graphics.drawString(font, font.plainSubstrByWidth(item.name, contentW - 18), listX + 3, itemY + 2, textColor, false);
            String meta = item.getDimensionName() + " " + item.getCoordinateName();
            graphics.drawString(font, font.plainSubstrByWidth(meta, contentW - 18), listX + 3, itemY + 12, metaColor, false);
            ...
            if ((item.type == WaypointEntry.Type.HOME || item.type == WaypointEntry.Type.STAGE) && item.canTeleport) {
                ...
                graphics.drawString(font, "×", delX + 2, delY, delHover ? COLORS_DELETE_HOVER : COLORS_TEXT_DIM, false);
            }
        }
        ...
        if (items.size() > MAX_VISIBLE_ITEMS && maxScroll > 0) {
            ...
            AbstractGuiUtils.fill(graphics, scrollbarX, thumbY, SCROLLBAR_WIDTH, thumbH, COLORS_SCROLLBAR);
        }
    }

    private void drawFooter(GuiGraphics graphics, int footerY, int mouseX, int mouseY) {
        int footerX = SCREEN_MARGIN;
        int footerW = width - 2 * SCREEN_MARGIN;

        AbstractGuiUtils.drawRoundedRect(graphics, footerX, footerY, footerW, FOOTER_HEIGHT, COLORS_BG_PANEL, 3);
        AbstractGuiUtils.drawRoundedRectOutLineRough(graphics, footerX, footerY, footerW, FOOTER_HEIGHT, 1, COLORS_FOOTER_BORDER, 3);
        AbstractGuiUtils.fill(graphics, footerX + 2, footerY, footerW - 4, 1, COLORS_ACCENT_SOFT);
        ...
        if (selectedItem != null) {
            ...
            graphics.drawString(font, font.plainSubstrByWidth(dimStr, leftZoneW), leftX, y0, COLORS_TEXT_DIM);
            graphics.drawString(font, font.plainSubstrByWidth(coordStr, leftZoneW), leftX, y0 + lineH, COLORS_TEXT_DIM, false);
            graphics.drawString(font, font.plainSubstrByWidth(distanceStr, leftZoneW), leftX, y0 + lineH * 2, COLORS_TEXT, false);

            // 传送消耗
            String costStr = calculateCostDisplay(selectedItem);
            if (costStr != null && !costStr.isEmpty()) {
                Text costText = Text.literal(costStr).graphics(graphics).font(font).color(Color.argb(COLORS_TEXT_DIM));
    ...
    private void drawDeleteConfirmOverlay(GuiGraphics graphics, int mouseX, int mouseY) {
        AbstractGuiUtils.fill(graphics, 0, 0, width, height, 0x60000000);
        ...
        AbstractGuiUtils.drawRoundedRect(graphics, dlgX, dlgY, dlgW, dlgH, COLORS_BG_PANEL, 6);
        AbstractGuiUtils.drawRoundedRectOutLineRough(graphics, dlgX, dlgY, dlgW, dlgH, 1, COLORS_BORDER, 6);
        ...
        graphics.drawString(font, title, dlgX + (dlgW - font.width(title)) / 2, dlgY + 15, COLORS_TEXT, false);
        graphics.drawString(font, msg, dlgX + (dlgW - font.width(msg)) / 2, dlgY + 35, COLORS_TEXT_DIM, false);
        ...
        AbstractGuiUtils.drawRoundedRect(graphics, cancelX, btnY, cancelW, 20, cancelHover ? 0xE8D0D8E0 : 0xE8C0C8D0, 4);
        AbstractGuiUtils.drawRoundedRect(graphics, deleteX, btnY, deleteW, 20, deleteHover ? 0xE8F0A0A8 : 0xE8E09098, 4);
        graphics.drawString(font, cancelStr, cancelX + (cancelW - font.width(cancelStr)) / 2, btnY + 6, COLORS_TEXT, false);
        graphics.drawString(font, deleteStr, deleteX + (deleteW - font.width(deleteStr)) / 2, btnY + 6, COLORS_TEXT, false);
    }
    ...
    public static class WaypointEntry {
        ...
        public String getDimensionName() {
            ...
            String key = "dim." + coordinate.dimension().identifier().toString().replaceAll(":", ".");
            if (I18nUtils.hasTranslation(EnumI18nType.WORD, key)) {
                return Component.transClient(EnumI18nType.WORD, key).toString();
            }
            return coordinate.dimension().identifier().toString();
        }
