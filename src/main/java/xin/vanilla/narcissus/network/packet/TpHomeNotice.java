        ...
        if (ctx.flow().isServerbound()) {
            ...
            ctx.workHandler().execute(() -> {
                ...
                ctx.player().ifPresent(player ->
                        Objects.requireNonNull(player.getServer()).getCommands()
                                .performPrefixedCommand(player.createCommandSourceStack(), NarcissusUtils.getCommand(ECommandType.TP_HOME))
                );
            });
        }
    }
